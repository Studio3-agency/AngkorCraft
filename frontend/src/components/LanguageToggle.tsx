...
export const LanguageToggle: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  return (
    ...
    >
      <Globe className="w-3.5 h-3.5 text-[#BF5A36] shrink-0" />
      <span className={language === 'en' ? 'text-[#BF5A36]' : 'text-[#8C7A70]'}>EN</span>
      <span className="text-[#8C7A70]/40">/</span>
      <span className={language === 'kh' ? 'text-[#BF5A36] font-sans' : 'text-[#8C7A70]'}>ខ្មែរ</span>
    </button>
  );
};
