        ...
        {products.map((p) => (
          <div key={p.id} className="bg-white rounded-2xl border border-[#E8DEC8] overflow-hidden flex flex-col">
            <div className="h-36 bg-[#FAF7F2] relative">
              ...
              {p.isFeatured && (
                <span className="absolute top-2 left-2 bg-[#FF914D] text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                  <Star className="w-3 h-3 fill-current" /> {t('featuredBadge')}
                </span>
              )}
            </div>
            <div className="p-4 flex-1 flex flex-col">
              <div className="text-[10px] font-bold text-[#FF914D] uppercase tracking-wider">{p.category}</div>
